...
class Post(models.Model):
    ...
    modified_time = models.DateTimeField('修改时间', default=None)
    ...
    # 新增 views 字段记录阅读量
    # PositiveIntegerField类型的值只允许为正整数或 0
...
class InviteCode(models.Model):
    '''
    实现邀请码注册机制
    模型创建完毕后，如需在后台管理员界面进行管理，则需打开admin.py注册模型
    '''
    invitecode = models.CharField(max_length=10, unique=True)
    state = models.IntegerField(default=0)
    # create_time = models.DateTimeField(auto_now_add=True)
    created_time = models.DateTimeField(default=timezone.now)
    used_by = models.ForeignKey(User, verbose_name='用户', on_delete=models.CASCADE, null=True)
    user_ip = models.CharField(max_length=30, blank=True)
